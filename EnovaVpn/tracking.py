...
# Start mitmproxy using subprocess (ensure mitmproxy is installed)
def start_mitmproxy():
    try:
        # Running mitmproxy in the background to capture traffic
        subprocess.Popen(["mitmproxy", "--mode", "http", "--listen-port", "8080"], stdout=subprocess.PIPE,
...
# Set up mitmproxy on device to capture traffic
start_mitmproxy()


def set_device_proxy():
    """Set the proxy on the device to route traffic through mitmproxy"""
    try:
        # Setting the device proxy to use the mitmproxy (localhost IP and port 8080)
        # Use your Windows machine's IP address, replace with your local IP address
        device_ip = "192.168.0.103"  # Replace with your PC's IP address
        proxy_port = "8080"  # Default port for mitmproxy

        # Set the proxy via ADB (Android Debug Bridge) command
        subprocess.run(f"adb shell settings put global http_proxy {device_ip}:{proxy_port}", shell=True)
        print(f"Proxy set on device: {device_ip}:{proxy_port}")
    except Exception as e:
        print(f"❌ Failed to set proxy on device: {e}")


# Set the proxy on the device
set_device_proxy()
